...
Theorem wp_goal :
  ...
  let a7 := memcpy t1 a5 t4 a5 x1 in
  let a8 := shift a2 x in
  ...
  ((region (base a)) <= 0%Z)%Z -> ((i2 + x)%Z <= x1)%Z ->
  ((i + (i * i3)%Z)%Z <= x2)%Z -> ((i * (1%Z + i3)%Z)%Z <= x2)%Z ->
  ((8%Z + i4)%Z <= i2)%Z -> linked t -> valid_rd t a3 x3 ->
  valid_rd t a4 x2 -> valid_rw t a5 x1 -> separated a3 x3 a5 x1 ->
  separated a4 x2 a5 x1 -> P_MatrixResult t1 a a1 a2 0%Z i i2 i1 ->
  valid_rw t a6 8%Z -> P_zeroed a7 a2 x x ->
  P_MatrixResult a7 a a1 a2 i3 i i2 i1 -> P_zeroed a9 a2 x x4 ->
  (forall (i5:Numbers.BinNums.Z), (0%Z <= i5)%Z -> (i5 <= 7%Z)%Z ->
   ((a10 i5) = (BuiltIn.IZR 0%Z))) ->
  (forall (i5:Numbers.BinNums.Z), (0%Z <= i5)%Z -> (i5 <= 7%Z)%Z ->
   ((a11 (shift a2 ((i4 + i5)%Z + x)%Z)) = (a10 i5))) ->
  P_zeroed a11 a2 x ((8%Z + i4)%Z + x)%Z.
Require Import Lia.
Proof.
