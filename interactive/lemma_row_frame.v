...
(* Why3 goal *)
Theorem wp_goal :
  forall (a:addr) (a1:addr) (a2:addr) (i:Numbers.BinNums.Z) (t2:addr -> t)
    (t3:addr -> t) (i1:Numbers.BinNums.Z) (i2:Numbers.BinNums.Z)
    (i3:Numbers.BinNums.Z),
  (0%Z < i1)%Z -> (0%Z <= i2)%Z -> (i3 <= i1)%Z ->
  P_RowResult t3 a a1 a2 i3 i2 i i1 ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a1 i4 in
   (0%Z <= i4)%Z -> (i4 < (i * i1)%Z)%Z ->
   ((t'real (t3 a3)) = (t'real (t2 a3)))) ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a i4 in
   (0%Z <= i4)%Z -> (i4 < (i * (1%Z + i2)%Z)%Z)%Z ->
   ((t'real (t3 a3)) = (t'real (t2 a3)))) ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a2 (i4 + (i1 * i2)%Z)%Z in
   (0%Z <= i4)%Z -> (i4 < i1)%Z -> ((t'real (t3 a3)) = (t'real (t2 a3)))) ->
  P_RowResult t2 a a1 a2 i3 i2 i i1.
Proof.
intros a a1 a2 i t2 t3 i1 i2 i3 h1 h2 h3 h4 h5 h6 h7.
induction h4.
- apply Q_P_RowResult_rowresult_empty. assumption.
- apply Q_P_RowResult_rowresult_step.
  + assumption.
  + apply IHh4; try assumption; try lia.
  + assert (Hdp: P_DotProduction t2 A B K i K x N (t'real (Mf32 (shift C (x + N * i)%Z)))).
    { apply Q_dp_frame with (Mf321:=Mf32); try assumption; try lia. }
    rewrite (h7 x ltac:(lia) ltac:(lia)) in Hdp.
    replace (- (1) + to)%Z with x by (unfold x; lia).
    exact Hdp.
Qed.
