...
Theorem wp_goal :
  ...
  P_RowResult t1 a a1 a2 i3 i2 i i1 ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a1 i4 in
   (0%Z <= i4)%Z -> (i4 < (i * i1)%Z)%Z -> ((t1 a3) = (t a3))) ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a i4 in
   (0%Z <= i4)%Z -> (i4 < (i * (1%Z + i2)%Z)%Z)%Z -> ((t1 a3) = (t a3))) ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a2 (i4 + (i1 * i2)%Z)%Z in
   (0%Z <= i4)%Z -> (i4 < i1)%Z -> ((t1 a3) = (t a3))) ->
  P_RowResult t a a1 a2 i3 i2 i i1.

Require Import Psatz.
Proof.
intros a a1 a2 i t t1 i1 i2 i3 h1 h2 h3 h4 h5 h6 h7.
induction h4.
- apply Q_P_RowResult_rowresult_empty. assumption.
- apply Q_P_RowResult_rowresult_step.
  + assumption.
  + apply IHh4; [nia|nia|nia|assumption|assumption|intros i5 H1 H2; apply h7; nia].
  + assert (Hc: Mf32 (shift C ((-(1)+to) + N*i)%Z) = t (shift C ((-(1)+to) + N*i)%Z)).
    { apply h7; nia. }
    rewrite <- Hc.
    apply Q_dp_frame with (Mf321:=Mf32).
    * lia.
    * nia.
    * nia.
    * nia.
    * exact H0.
    * intros i5 H1 H2. apply h5; nia.
    * intros i5 H1 H2. apply h6; nia.
Qed.
