...
Axiom eb_gt_1 : (1%Z < 8%Z)%Z.

Axiom sb_gt_1 : (1%Z < 24%Z)%Z.
...
Axiom pow2sb : (16777216%Z = (bv.Pow2int.pow2 24%Z)).
...
Parameter from_real: mode -> Reals.Rdefinitions.R -> t.

Axiom from_real_in_range :
  forall (m:mode) (r:Reals.Rdefinitions.R), in_range (round m r) ->
  let f := from_real m r in t'isFinite f /\ ((t'real f) = (round m r)).

Axiom from_real_large_neg :
  forall (m:mode) (r:Reals.Rdefinitions.R),
...
Axiom eb_gt_11 : (1%Z < 11%Z)%Z.

Axiom sb_gt_11 : (1%Z < 53%Z)%Z.
...
Axiom pow2sb1 : (9007199254740992%Z = (bv.Pow2int.pow2 53%Z)).
...
Parameter from_real1: mode -> Reals.Rdefinitions.R -> t1.

Axiom from_real_in_range1 :
  forall (m:mode) (r:Reals.Rdefinitions.R), in_range1 (round1 m r) ->
  let f := from_real1 m r in t'isFinite1 f /\ ((t'real1 f) = (round1 m r)).

Axiom from_real_large_neg1 :
  forall (m:mode) (r:Reals.Rdefinitions.R),
  ((round1 m r) <
   (-%R)%R)%R ->
  let f := from_real1 m r in is_infinite1 f /\ is_negative1 f.

Axiom from_real_large_pos1 :
  forall (m:mode) (r:Reals.Rdefinitions.R),
  (%R
   < (round1 m r))%R ->
  let f := from_real1 m r in is_infinite1 f /\ is_positive1 f.
...
Inductive sign :=
  | Positive : sign
  | Negative : sign.
Axiom sign_WhyType : WhyType sign.
Existing Instance sign_WhyType.
...
Axiom to_f32_minus_infinity :
  forall (x:Reals.Rdefinitions.R),
  (x < (-340282346638528859811704183484516925440%R)%R)%R ->
  is_minus_infinity (to_f32 x).

Axiom to_f32_plus_infinity :
  forall (x:Reals.Rdefinitions.R),
  (340282346638528859811704183484516925440%R < x)%R ->
...
Parameter sign_f32: t -> sign.

Axiom positive_f32_sign :
  forall (f:t), is_positive f <-> ((sign_f32 f) = Positive).

Axiom negative_f32_sign :
  forall (f:t), is_negative f <-> ((sign_f32 f) = Negative).

Parameter sign_f64: t1 -> sign.

Axiom positive_f64_sign :
  forall (f:t1), is_positive1 f <-> ((sign_f64 f) = Positive).

Axiom negative_f64_sign :
  forall (f:t1), is_negative1 f <-> ((sign_f64 f) = Negative).

(* Why3 assumption *)
Definition P_zeroed (Mf32:addr -> t) (a:addr) (b:Numbers.BinNums.Z)
    (e:Numbers.BinNums.Z) : Prop :=
  forall (i:Numbers.BinNums.Z), (b <= i)%Z -> (i < e)%Z ->
  ((t'real (Mf32 (shift a i))) = 0%R).

(* Why3 assumption *)
Inductive P_DotProduction: (addr -> t) -> addr -> addr ->
  ...
  | Q_P_DotProduction_dotproduction_positive_range :
      ...
      (r +
       ((t'real (Mf32 (shift a (((-1%Z)%Z + i)%Z + (i1 * i2)%Z)%Z))) *
        (t'real (Mf32 (shift a1 (i3 + (i4 * x)%Z)%Z))))%R)%R.

(* Why3 assumption *)
Inductive P_RowResult: (addr -> t) -> addr -> addr -> addr ->
  Numbers.BinNums.Z -> Numbers.BinNums.Z -> Numbers.BinNums.Z ->
  Numbers.BinNums.Z -> Prop :=
  | Q_P_RowResult_rowresult_empty :
      forall (Mf32:addr -> t) (A:addr) (B:addr) (C:addr)
  ...
  | Q_P_RowResult_rowresult_step :
      ...
      (t'real (Mf32 (shift C (((-1%Z)%Z + to)%Z + (N * i)%Z)%Z))) ->
      P_RowResult Mf32 A B C to i K N.

(* Why3 assumption *)
Inductive P_MatrixResult: (addr -> t) -> addr -> addr -> addr ->
  Numbers.BinNums.Z -> Numbers.BinNums.Z -> Numbers.BinNums.Z ->
  Numbers.BinNums.Z -> Prop :=
...
Axiom Q_row_frame :
  forall (Mf32:addr -> t) (Mf321:addr -> t) (A:addr) (B:addr) (C:addr)
  ...
  (forall (i:Numbers.BinNums.Z),
   let a := shift A i in
   (0%Z <= i)%Z -> (i < (K * (1%Z + row)%Z)%Z)%Z ->
   ((t'real (Mf321 a)) = (t'real (Mf32 a)))) ->
  (forall (i:Numbers.BinNums.Z),
   let a := shift B i in
   (0%Z <= i)%Z -> (i < (K * N)%Z)%Z ->
   ((t'real (Mf321 a)) = (t'real (Mf32 a)))) ->
  (forall (i:Numbers.BinNums.Z),
   ...
   (0%Z <= i)%Z -> (i < N)%Z -> ((t'real (Mf321 a)) = (t'real (Mf32 a)))) ->
  P_RowResult Mf32 A B C to row K N.

Axiom Q_dp_frame :
  forall (Mf32:addr -> t) (Mf321:addr -> t) (A:addr) (B:addr)
    (to:Numbers.BinNums.Z) (i:Numbers.BinNums.Z) (K:Numbers.BinNums.Z)
...
Theorem wp_goal :
  forall (a:addr) (a1:addr) (a2:addr) (i:Numbers.BinNums.Z)
    (i1:Numbers.BinNums.Z) (t2:addr -> t) (t3:addr -> t)
    (i2:Numbers.BinNums.Z) (i3:Numbers.BinNums.Z),
  (0%Z < i2)%Z -> (0%Z < i)%Z -> (i3 <= i1)%Z ->
  P_MatrixResult t3 a a1 a2 i3 i i2 i1 ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a2 i4 in
   (0%Z <= i4)%Z -> (i4 < (i2 * i3)%Z)%Z ->
   ((t'real (t3 a3)) = (t'real (t2 a3)))) ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a1 i4 in
   (0%Z <= i4)%Z -> (i4 < (i * i2)%Z)%Z ->
   ((t'real (t3 a3)) = (t'real (t2 a3)))) ->
  (forall (i4:Numbers.BinNums.Z),
   let a3 := shift a i4 in
   (0%Z <= i4)%Z -> (i4 < (i * i3)%Z)%Z ->
   ((t'real (t3 a3)) = (t'real (t2 a3)))) ->
  P_MatrixResult t2 a a1 a2 i3 i i2 i1.
Proof.
intros a a1 a2 i i1 t2 t3 i2 i3 h1 h2 h3 h4 h5 h6 h7.
assert (Hgen: forall to0:Numbers.BinNums.Z, P_MatrixResult t3 a a1 a2 to0 i i2 i1 -> (to0<=i3)%Z ->
              P_MatrixResult t2 a a1 a2 to0 i i2 i1).
{ intros to0 Hp.
  induction Hp; intro Hbound.
  - apply Q_P_MatrixResult_matrixresult_empty. assumption.
  - apply Q_P_MatrixResult_matrixresult_step.
    + assumption.
    + apply Q_row_frame with (Mf321:=Mf32).
      * assumption.
      * lia.
      * lia.
      * assumption.
      * intros i4 a3 Hi4a Hi4b. apply h7; [assumption | nia].
      * intros i4 a3 Hi4a Hi4b. apply h6; [assumption | nia].
      * intros i4 a3 Hi4a Hi4b. apply h5; nia.
    + apply IHHp; try assumption; try lia.
}
apply Hgen; [exact h4 | lia].
Qed.
