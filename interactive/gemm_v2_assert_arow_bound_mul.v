...
Theorem wp_goal :
  ...
  (0%Z < i)%Z -> (0%Z <= i3)%Z -> (i3 < i1)%Z -> (i3 <= i1)%Z ->
  ((region (base a2)) <= 0%Z)%Z -> ((region (base a1)) <= 0%Z)%Z ->
  ((region (base a)) <= 0%Z)%Z -> ((i2 + (i2 * i3)%Z)%Z <= x)%Z ->
  linked t -> valid_rd t a3 x1 -> valid_rd t a4 x2 -> valid_rw t a5 x ->
  separated a3 x1 a5 x -> separated a4 x2 a5 x ->
  P_MatrixResult t1 a a1 a2 0%Z i i2 i1 ->
  P_MatrixResult (memcpy t1 a5 t2 a5 x) a a1 a2 i3 i i2 i1 ->
  ((i * (1%Z + i3)%Z)%Z <= x2)%Z.
Require Import Lia.
Proof.
intros a a1 a2 i i1 t t1 t2 i2 i3 x a3 x1 a4 x2 a5 h1 h2 h3 h4 h5 h6 h7 h8 h9
h10 h11 h12 h13 h14 h15 h16 h17 h18 h19.
(* Цель: i*(1+i3) <= x2, где x2 = i*i1. Это ровно mul_mono_nonneg,
   применённая к (1+i3)<=i1 (из h6: i3<i1) и 0<=i (из h4: 0<i) — только
   умножение записано в другом порядке (i * _ вместо _ * i), поэтому
   сначала коммутируем обе стороны. *)
unfold x2.
rewrite (ZArith.BinInt.Z.mul_comm i (1 + i3)).
rewrite (ZArith.BinInt.Z.mul_comm i i1).
apply Q_mul_mono_nonneg; lia.
Qed.
