...
Theorem wp_goal :
  ...
  (x1 <= x2)%Z -> ((i + (i * i3)%Z)%Z <= x3)%Z -> ((i * x4)%Z <= x3)%Z ->
  linked t -> valid_rd t a3 x5 -> valid_rd t a4 x3 -> valid_rw t a5 x2 ->
  separated a3 x5 a5 x2 -> separated a4 x3 a5 x2 ->
  P_MatrixResult t1 a a1 a2 0%Z i i2 i1 -> P_zeroed a6 a2 x x ->
  P_MatrixResult a6 a a1 a2 i3 i i2 i1 -> P_zeroed a8 a2 x (i4 + x)%Z ->
  P_zeroed a8 a2 x x1 -> P_RowResult a9 a a1 a2 i2 i3 i i2 ->
  ...
  P_MatrixResult a9 a a1 a2 x4 i i2 i1.
(* Why3 intros a a1 a2 i i1 t t1 t2 t3 t4 i2 i3 i4 i5 x x1 x2 x3 x4 a3 x5 a4
        a5 a6 a7 a8 a9 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14 h15 h16
        h17 h18 h19 h20 h21 h22 h23 h24 h25 h26 h27 h28 h29 h30 h31 h32 h33
        h34. *)
Require Import Psatz.
Proof.
intros a a1 a2 i i1 t t1 t2 t3 t4 i2 i3 i4 i5 x x1 x2 x3 x4 a3 x5 a4 a5 a6 a7
a8 a9 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14 h15 h16 h17 h18 h19 h20
h21 h22 h23 h24 h25 h26 h27 h28 h29 h30 h31 h32 h33 h34.

(* ---- A, B unaffected by the row's memcpy writes into C ---- *)
assert (EqA_a8: forall idx:Z, (0<=idx)%Z -> (idx<x3)%Z -> a8 (shift a idx) = a6 (shift a idx)).
{ intros idx H1 H2. unfold a8.
  destruct (memcpy'def a6 a7 t3 a7 i2 (shift a idx)) as [Hs _]. apply Hs.
  destruct a as [ba oa]. destruct a2 as [ba2 oa2].
  unfold a7, a4, a5, separated, x, x2, x3 in *; cbn [base offset shift] in *.
  destruct h26 as [Hb|[Hb|[Hb|[Hb|Hb]]]]; nia. }

assert (EqA_a9: forall idx:Z, (0<=idx)%Z -> (idx<x3)%Z -> a9 (shift a idx) = a8 (shift a idx)).
{ intros idx H1 H2. unfold a9.
  destruct (memcpy'def a8 a7 t2 a7 i2 (shift a idx)) as [Hs _]. apply Hs.
  destruct a as [ba oa]. destruct a2 as [ba2 oa2].
  unfold a7, a4, a5, separated, x, x2, x3 in *; cbn [base offset shift] in *.
  destruct h26 as [Hb|[Hb|[Hb|[Hb|Hb]]]]; nia. }

assert (EqA: forall idx:Z, (0<=idx)%Z -> (idx<x3)%Z -> a9 (shift a idx) = a6 (shift a idx)).
{ intros idx H1 H2. rewrite (EqA_a9 idx H1 H2). rewrite (EqA_a8 idx H1 H2). reflexivity. }

assert (EqB_a8: forall idx:Z, (0<=idx)%Z -> (idx<x5)%Z -> a8 (shift a1 idx) = a6 (shift a1 idx)).
{ intros idx H1 H2. unfold a8.
  destruct (memcpy'def a6 a7 t3 a7 i2 (shift a1 idx)) as [Hs _]. apply Hs.
  destruct a1 as [ba oa]. destruct a2 as [ba2 oa2].
  unfold a7, a3, a5, separated, x, x2, x5 in *; cbn [base offset shift] in *.
  destruct h25 as [Hb|[Hb|[Hb|[Hb|Hb]]]]; nia. }

assert (EqB_a9: forall idx:Z, (0<=idx)%Z -> (idx<x5)%Z -> a9 (shift a1 idx) = a8 (shift a1 idx)).
{ intros idx H1 H2. unfold a9.
  destruct (memcpy'def a8 a7 t2 a7 i2 (shift a1 idx)) as [Hs _]. apply Hs.
  destruct a1 as [ba oa]. destruct a2 as [ba2 oa2].
  unfold a7, a3, a5, separated, x, x2, x5 in *; cbn [base offset shift] in *.
  destruct h25 as [Hb|[Hb|[Hb|[Hb|Hb]]]]; nia. }

assert (EqB: forall idx:Z, (0<=idx)%Z -> (idx<x5)%Z -> a9 (shift a1 idx) = a6 (shift a1 idx)).
{ intros idx H1 H2. rewrite (EqB_a9 idx H1 H2). rewrite (EqB_a8 idx H1 H2). reflexivity. }

(* ---- C rows before row i3 are unaffected by this row's memcpy writes ---- *)
assert (EqC_a8: forall idx:Z, (0<=idx)%Z -> (idx<x)%Z -> a8 (shift a2 idx) = a6 (shift a2 idx)).
{ intros idx H1 H2. unfold a8.
  destruct (memcpy'def a6 a7 t3 a7 i2 (shift a2 idx)) as [Hs _]. apply Hs.
  unfold a7, separated in *; cbn [base offset shift] in *.
  right; right; right; right; nia. }

assert (EqC_a9: forall idx:Z, (0<=idx)%Z -> (idx<x)%Z -> a9 (shift a2 idx) = a8 (shift a2 idx)).
{ intros idx H1 H2. unfold a9.
  destruct (memcpy'def a8 a7 t2 a7 i2 (shift a2 idx)) as [Hs _]. apply Hs.
  unfold a7, separated in *; cbn [base offset shift] in *.
  right; right; right; right; nia. }

assert (EqC: forall idx:Z, (0<=idx)%Z -> (idx<x)%Z -> a9 (shift a2 idx) = a6 (shift a2 idx)).
{ intros idx H1 H2. rewrite (EqC_a9 idx H1 H2). rewrite (EqC_a8 idx H1 H2). reflexivity. }

(* ---- one call to the ready-made lemma instead of a from-scratch induction ---- *)
assert (HMatTransfer: P_MatrixResult a9 a a1 a2 i3 i i2 i1).
{ apply Q_mat_frame with (Mf321:=a6).
  - lia.
  - lia.
  - lia.
  - exact h29.
  - intros idx aa H1 H2. symmetry. apply EqA; [assumption | nia].
  - intros idx aa H1 H2. symmetry. apply EqB; [assumption | nia].
  - intros idx aa H1 H2. symmetry. apply EqC; [assumption | nia].
}

assert (Hpos_x4: (0 < x4)%Z) by (unfold x4; lia).
assert (Hrow_eq: ((- (1))%Z + x4)%Z = i3) by (unfold x4; lia).
assert (HRow: P_RowResult a9 a a1 a2 i2 ((- (1))%Z + x4)%Z i i2).
{ rewrite Hrow_eq. exact h32. }
assert (HMat_prev: P_MatrixResult a9 a a1 a2 ((- (1))%Z + x4)%Z i i2 i1).
{ rewrite Hrow_eq. exact HMatTransfer. }
exact (Q_P_MatrixResult_matrixresult_step a9 a a1 a2 x4 i i2 i1 Hpos_x4 HRow HMat_prev).
Qed.
