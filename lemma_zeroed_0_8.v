...
Axiom Q_zeroed_shift_inv :
  forall (Mf32:addr -> Reals.Rdefinitions.R) (A:addr)
    (shift1:Numbers.BinNums.Z) (b:Numbers.BinNums.Z) (e:Numbers.BinNums.Z),
  (0%Z <= b)%Z -> (b < e)%Z -> (0%Z <= shift1)%Z ->
  P_zeroed Mf32 A (b + shift1)%Z (e + shift1)%Z ->
...
(* Why3 goal *)
Theorem wp_goal :
  forall (t:addr -> Reals.Rdefinitions.R) (a:addr) (i:Numbers.BinNums.Z),
  (0%Z <= i)%Z -> P_zeroed t (shift a i) 0%Z 8%Z ->
  P_zeroed t a i (8%Z + i)%Z.
Proof.
intros t a i h1 h2.
unfold P_zeroed.
intros i1.
intros Hii1.
intros Hi1i.
unfold P_zeroed in h2.
specialize (h2 (i1 - i)%Z).
assert (Hlb : (0 <= i1 - i)%Z) by lia.
specialize (h2 Hlb).
assert (Hub : (i1 - i < 8)%Z) by lia.
specialize (h2 Hub).
rewrite shift_compose in h2.
assert (Hcancel: shift a (i + (i1 - i)) = shift a i1).
{
assert (Obvious: (i + (i1 - i)%Z)%Z = i1) by lia.
rewrite Obvious.
reflexivity.
}
rewrite Hcancel in h2.
- exact h2.
- exact h1.
- exact Hlb.
Qed.
